...
# Replace substring: ${main_string/search_term/replace_term}
...
&& echo "<tr><td>0.$CLASP_VERSION_BUMP.0</td><td>$PUBLISH_DATE_FOR_HISTORY</td><td>$GCC_MESSAGE.</td></tr>" >> $TAGS_FILE \
...
&& npm version $NPM_VERSION_BUMP --message "$GCC_MESSAGE" \
&& npm run push \
&& npx clasp version "$GCC_MESSAGE" \
&& npx clasp deploy --description "$GCC_MESSAGE" --deploymentId $DEPLOYMENT_ID --versionNumber $CLASP_VERSION_BUMP \
